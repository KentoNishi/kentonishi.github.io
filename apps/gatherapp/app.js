...
function action(act){
  ...
  if(uid!=null&&uid.length>1&&loaded){
    if(act=="menu"){
      var card="<div class='card'><span style='font-size:5vh'>"+name+"</span><br /><img class='pic' src='"+pic+"'></img><br /><a href='mailto:"+email+"'>@"+email.split("@")[0]+"</a><br /><textarea spellcheck='false' onkeyup='auto_grow(this)' style='text-align:center;font-size:4vh;' onkeypress='enter(event);return alpha(event);' maxlength='144' >"+desc+"</textarea><br /><a href='javascript:signOut();'>Sign Out</a></div>";
      ...
      for(var i=circles.split(",").length-1;i>-1;i--){
        card+="<br /><div class='card'><span style='font-size:5vh'>"+groups[i]+"</span></div>";
      }
      document.querySelectorAll(".body")[0].innerHTML=card;
      auto_grow(document.querySelectorAll("textarea")[0]);
    }
