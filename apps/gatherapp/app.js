        ...
        firebase.database().ref('users/'+uid+"/groups/").once('value', function(snapshot){
            snapshot.forEach(function(childSnapshot) {
                ...
                firebase.database().ref('groups/'+myGroups[i]).once('value', function(snap) {
                    write(snap.val().info.group,(snap.val().info.desc||"Description Here"),"javascript:if(confirmLeave('"+encodeURIComponent(snap.val().info.group)+"')){group('"+snap.key+"','leave');remove('users/"+uid+"/groups/"+snap.key+"','action', 'add');groups(uid);}","Leave Group","load('"+childSnapshot.val().group+"');",snap.val().stats.popularity+" members");
                });
                i++;
            });
        });
